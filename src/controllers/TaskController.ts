...
// Importamos el servicio y la interfaz de filtros (asumiendo que ITaskFilters ahora se exporta desde TaskService.ts)
import { TaskService, ITaskFilters } from "../services/TaskService"; 

export class TaskController {
  
  // --- MÉTODO GETALL COMPLETO CON TODOS LOS FILTROS ---
  static async getAll(req: Request, res: Response) {
    try {
      ...
      // Lógica para Etiquetas: convierte el string "1,2,3" a array [1, 2, 3]
      if (tags && typeof tags === 'string') {
        filters.tagIds = tags.split(',').map(id => Number(id));
      }
      
      const taskService = new TaskService();
      // El servicio TaskService.getAllTasks() maneja la lógica de QueryBuilder con todos los filtros
  ...
  // (getOneById - Sigue igual, ya incluye tags)
